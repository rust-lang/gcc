...
print "  0\n};\n"
if (mt_pin_sentinel != "")
	print "const unsigned int cl_options_count = N_OPTS_REAL;\n"
else
...
# The active-table pointers: the shared options.cc defines them
# pointing at its own tables, so every program linking an option
# table starts on the primary's, with no initialization-order
# concerns; activation repoints them, and opts.h routes the other
# units' references through them.
if (multi_target != "" && mt_prefix == "") {
	print ""
	print "#if ENABLE_MULTI_TARGET"
	print "const struct cl_option *mt_active_cl_options = cl_options;"
	print "unsigned int mt_active_cl_options_count = cl_options_count;"
	print "const struct cl_enum *mt_active_cl_enums = cl_enums;"
	print "unsigned int mt_active_cl_enums_count = cl_enums_count;"
	print "const unsigned short *mt_active_cl_option_name_order"
	print "  = cl_option_name_order;"
	print "const struct gcc_options *mt_active_global_options_init"
	print "  = &global_options_init;"
	print "bool (*mt_active_common_handle_option_auto)"
	print "  (struct gcc_options *, struct gcc_options *,"
	print "   const struct cl_decoded_option *, unsigned int, int,"
	print "   location_t, const struct cl_option_handlers *,"
	print "   diagnostics::context *) = common_handle_option_auto;"
	print "void (*mt_active_cpp_handle_option_auto)"
	print "  (const struct gcc_options *, size_t, struct cpp_options *)"
	print "  = cpp_handle_option_auto;"
